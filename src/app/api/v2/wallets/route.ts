...
export async function POST(req: Request) {
  return handle(
    ...
    async (c, user, context) => {
      ...
      const input = parseWallet(context.input!);
      return idempotent(c, user, key, 'wallet.create', input, () =>
        createWallet(c, user, input, context.requestId)
      );
    },
    { rateLimitScope: 'wallet:create' }
  );
}
