...
- [x] **Client-Side Model Enforcement**: Immutable wallet types, credit-limit debt enforcement, system transaction protection (BILL/GOAL immutability).
- [x] **API Guards**: Mutation APIs reject arbitrary payloads, missing required fields, and oversized requests (`400 Bad Request`).
- [x] **Demo-Only Indication**: All Mock API endpoints respond with `X-Demo-Only: true`.
...
- [x] **Snapshot Schema Validation**: Strict type, enum, number range, and referential integrity checks via `storage-schema.ts`.
...
- [x] **CSV/Spreadsheet Formula Injection Defense**: All user-controlled fields (`note`, `categoryName`, `walletName`, `toWalletName`) prefixed with `\t` if starting with `=`, `+`, `-`, or `@`.
- [x] **Receipt File Validation**: MIME type allowlist (`image/jpeg`, `image/png`, `image/webp`), SVG disallowed, size cap (5 MB) enforced via `validateReceiptFile`.
- [x] **Payload Size Limits**: Max 5 MB import limit in AppContext and 1 MB body size cap in API routes.
...
- [x] **Safe Recovery Keying**: Corrupted snapshots never overwritten; safely duplicated to `fintrack_recovery_corrupt_<timestamp>`.
...
- [x] **Storage Failure Transparency**: Context status (`SAVE_ERROR`, `RECOVERY_REQUIRED`) clearly presented via `StorageStatusBanner`.
...
- [x] `Content-Security-Policy`: Restricts scripts, styles, objects, and framing (`frame-ancestors 'none'`).
...
- [x] `X-Frame-Options: DENY`: Prevents clickjacking in legacy user agents.
